...
export async function refreshInstalledSkillCache(
  cache: InstalledSkillCache,
  list: () => Promise<readonly { name: string; invocation?: { modelInvocable?: boolean } }[]>,
  onError: (error: unknown) => void,
): Promise<void> {
  try {
    await cache.refresh(list)
  } catch (error) {
    onError(error)
  }
}
